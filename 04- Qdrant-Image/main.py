...
async def image_search(image_url: str=Form(...), top_k: int=Form(2000), threshold: float=Form(0.5)):
...
async def indexing_or_deleting_images(image_id: int=Form(...), image_url: str=Form(None), 
    ...
    if case == 'upsert':
        ...
        message = index_vectorDB(image_id=image_id, image_url=image_url, folder_path=INDEXING_IMAGES_FOLDER_PATH)
